import { fetchFilmForumSchedule } from "../src/ingestion/adapters/film-forum";

const [, , windowStart, windowEnd] = process.argv;
if (!/^\d{4}-\d{2}-\d{2}$/.test(windowStart ?? "") || !/^\d{4}-\d{2}-\d{2}$/.test(windowEnd ?? "")) {
  console.error(
    "Usage: npm run ingest:film-forum -- YYYY-MM-DD YYYY-MM-DD",
  );
  process.exit(2);
}

const result = await fetchFilmForumSchedule(windowStart, windowEnd);
console.log(
  JSON.stringify(
    {
