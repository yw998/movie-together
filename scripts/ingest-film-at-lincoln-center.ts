import { fetchFilmAtLincolnCenterSchedule } from "../src/ingestion/adapters/film-at-lincoln-center";

const [, , windowStart, windowEnd] = process.argv;
if (!/^\d{4}-\d{2}-\d{2}$/.test(windowStart ?? "") || !/^\d{4}-\d{2}-\d{2}$/.test(windowEnd ?? "")) {
  console.error("Usage: npm run ingest:film-at-lincoln-center -- YYYY-MM-DD YYYY-MM-DD");
  process.exit(2);
}
const result = await fetchFilmAtLincolnCenterSchedule(windowStart, windowEnd);
