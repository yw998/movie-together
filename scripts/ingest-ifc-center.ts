import { fetchIfcCenterSchedule } from "../src/ingestion/adapters/ifc-center";

const [, , windowStart, windowEnd] = process.argv;
if (!/^\d{4}-\d{2}-\d{2}$/.test(windowStart ?? "") || !/^\d{4}-\d{2}-\d{2}$/.test(windowEnd ?? "")) {
  console.error("Usage: npm run ingest:ifc-center -- YYYY-MM-DD YYYY-MM-DD");
  process.exit(2);
}

const result = await fetchIfcCenterSchedule(windowStart, windowEnd);
console.log(
  JSON.stringify(
    {
