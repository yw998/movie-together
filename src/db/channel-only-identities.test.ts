...
describe("Channel-only identities", () => {
  ...
  it("atomically transfers membership, ownership, marks, and shares to a formal account", async () => {
    const migration = await readFile(migrationPath, "utf8");
    ...
    expect(migration).toContain("on conflict (user_id, window_start, showing_id) do nothing");
  ...
  it("revokes legacy guests and deletes inactive identity-owned Channels", async () => {
    const migration = await readFile(migrationPath, "utf8");

    expect(migration).toContain("update channel_guests set revoked_at = coalesce(revoked_at, now())");
    expect(migration).toContain("cleanup_inactive_channel_identity_channels");
    expect(migration).toContain("interval '180 days'");
    expect(migration).not.toContain("interval '210 days'");
  });
  ...
  it("exposes the complete create, join, login, mark, management, and merge API", async () => {
    ...
    for (const action of [
      ...
      "identity_revoke_link", "identity_remove_member", "identity_leave",
